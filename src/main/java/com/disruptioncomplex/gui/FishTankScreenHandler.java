...
public class FishTankScreenHandler extends ScreenHandler {
    ...
    private static class FishTankResultSlot extends Slot {
        public FishTankResultSlot(Inventory inventory, int index, int x, int y) {
    ...
    public FishTankScreenHandler(int syncId, PlayerInventory playerInventory, Inventory inventory) {
        super(ModScreenHandlers.FISH_TANK_SCREEN_HANDLER, syncId);
        ...
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 6; col++) {
                ...
                this.addSlot(new FishTankResultSlot(inventory, index++, x, y));
            }
        }
